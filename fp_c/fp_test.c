...
void run_experiment(char *op)
{
    ...
    long *math_times = (long*) malloc(sizeof(long) * EXEC_COUNT);
    long *mpfr_times = (long*) malloc(sizeof(long) * EXEC_COUNT);
    ...
    if (strcmp(op, "add") == 0)
    {
        ...
        for (int i = 0; i < EXEC_COUNT; i++) {
            ...
            math_times[i] = (end.tv_sec - start.tv_sec) * 1e9;
            math_times[i] += (end.tv_nsec - start.tv_nsec);
        }
        /*for (int i = 0; i < WARMUP_COUNT; i++) {*/
            /*idx = i % INPUT_COUNT;*/
        ...
        for (int i = 0; i < EXEC_COUNT; i++) {
            ...
            mpfr_add(m_val, mxs[idx], mys[idx], MPFR_RNDN);
            clock_gettime(CLOCK_MONOTONIC_RAW, &end);
            mpfr_times[i] = (end.tv_sec - start.tv_sec) * 1e9;
            mpfr_times[i] += (end.tv_nsec - start.tv_nsec);
        }
    }
    ...
    print_measurements(op, math_times, "../../results/transc/c/math_");
    print_measurements(op, mpfr_times, "../../results/transc/c/mpfr_");
    // Clean-up
    free(xs);
    free(ys);
    free(math_times);
